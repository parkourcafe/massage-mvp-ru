    ...
    <div className="space-y-5">
      <div className="card">
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-sm">
          <span className="badge">Статус: {booking.status}</span>
          {booking.outcome && (
            <span className="chip-brand">Итог: {booking.outcome}</span>
          )}
      ...
      <div className="card">
        ...
        <div className="mt-4 space-y-3 max-h-96 overflow-y-auto">
          {messages.length === 0 && (
            <p className="small">Сообщений пока нет.</p>
          )}
          {messages.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className={`max-w-[80%] rounded-[var(--r-control)] px-4 py-2.5 text-sm ${
                  mine
                    ? "ml-auto bg-gradient-to-br from-accent to-plum-700 text-white"
                    : "surface text-body"
                }`}
              >
                <p className="text-xs opacity-70 mb-0.5">
                  {m.sender_name ||
                    (m.sender_type === "therapist"
                      ? "Специалист"
                      : "Клиент")}
                </p>
                {m.body}
      ...
      <div className="card space-y-3">
        <span className="eyebrow">
          {role === "therapist" ? "Итог сеанса" : "Управление записью"}
        </span>
